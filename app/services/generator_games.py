...
class GameGenerator:
    """Генератор игр для использования при отсутствии подключения к Steam"""
    ...
    def generate_game(game_id: int = None, currency: str = "RUB") -> Dict:
        ...
        # С вероятностью 5% делает игру бесплатной
    ...
    def get_featured_games(count: int = 24, currency: str = "RUB") -> List[Dict]:
        """Получение популярных игр (имитация главной страницы)"""
        games = []
        ...
        for i in range(count):
            ...
            # Для популярных игр используются более высокие цены
